...
DATA_FILE = os.path.join(os.path.dirname(__file__), "..", "test_data", "extra_documents.json")
    ...
    for i, doc in enumerate(docs):
        ...
        try:
            ...
            if data.get("code") == 0:
                ...
                total_chunks += chunks
                success += 1
                print(f"  [{i+1}/{len(docs)}] ✓ {doc['title'][:30]}... ({chunks} chunks)")
            else:
                failed += 1
                print(f"  [{i+1}/{len(docs)}] ✗ {doc['title'][:30]}... : {data.get('message', 'unknown')}")

        except Exception as e:
            failed += 1
            print(f"  [{i+1}/{len(docs)}] ✗ {doc['title'][:30]}... : {e}")
